...
fact_judge: Any = None # 사실 확인 판단 모듈.
...
class FactJudgeSignature(dspy.Signature):
    ...
    향후 찬성&반대 에이전트가 이 판단을 바탕으로 찬반 토론을 진행할 수 있도록
    판단 근거와 판정 이유를 상세히 써주세요.
    """

    claim: str = dspy.InputField( desc="판단할 주장" )
    evidence: str = dspy.InputField( desc="판단 근거가 되는 검색 결과 텍스트. 상세하게 작성." )
    verdict: str = dspy.OutputField( desc="판정 결과. 반드시 TRUE, FALSE, UNVERIFIABLE 중 하나만 출력." )
    reason: str = dspy.OutputField( desc="판정 이유. 상세하게 작성." )

class AgentDebateSignature(dspy.Signature):
    """
    찬성 에이전트와 반대 에이전트가 주어진 주장과 근거를 바탕으로 각각 논거를 제시.
    """
...
class LLMJudgeSignature(dspy.Signature):
    ...
    reason: str = dspy.InputField(desc="판정 이유")
...
_TRAINING_DATA: list[dict] = [
    {
        ...
        # FactJudge/LLMJudge/DebateAgent 입력: text에서 추출된 주장 하나
...
def setup_dspy(openai_api_key: str, model_name: str) -> None:
    ...
    global claim_extractor, fact_judge, llm_judge, agent_debate, debate_judge
    ...
    # 주장(claim)과 근거(evidence)를 가탕으로 사실인지(verdict) 판단하고, 그 이유(reason)를 컴파일.
    # 입력값은 주장과 근거
    fact_judge_trainset = []
    for d in _TRAINING_DATA:
        example = dspy.Example(claim=d["claim"], evidence=d["evidence"], verdict=d["verdict"], reason=d["reason"]).with_inputs("claim", "evidence")
    ...
    for d in _TRAINING_DATA:
        example = dspy.Example( claim=d["claim"], evidence=d["evidence"], debate_pro=d["debate_pro"], debate_con=d["debate_con"], verdict=d["verdict"], reason=d["reason"]).with_inputs("claim", "evidence", "debate_pro", "debate_con")
    ...
    llm_judge_trainset = []
    for d in _TRAINING_DATA:
        example = dspy.Example(claim=d["claim"], evidence=d["evidence"], verdict=d["verdict"], reason=d["reason"], quality_score=d["quality_score"], feedback=d["feedback"]).with_inputs("claim", "evidence", "verdict", "reason")
